...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Debug endpoint to show raw API data for specific property
  app.get('/api/debug/property/:propertyIndex?', async (req, res) => {
    try {
      const propertyIndex = parseInt(req.params.propertyIndex || '0');
      const location = req.query.location || 'Orlando, FL';
      
      console.log(`🔍 Fetching raw API data for property index ${propertyIndex} in ${location}...`);
      
      const { batchLeadsService } = await import("./batchleads");
      
      // Get multiple pages to find the specific property index
      const pageSize = 50;
      const targetPage = Math.floor(propertyIndex / pageSize) + 1;
      const indexOnPage = propertyIndex % pageSize;
      
      console.log(`📄 Looking on page ${targetPage}, index ${indexOnPage}`);
      ...
      if (!response.data || response.data.length === 0) {
        return res.json({
          error: `No properties found on page ${targetPage}`,
          totalResults: response.total_results,
          requestedIndex: propertyIndex
        });
      }
      
      if (indexOnPage >= response.data.length) {
        return res.json({
          error: `Property index ${indexOnPage} not found on page ${targetPage} (only ${response.data.length} properties on this page)`,
          totalResults: response.total_results,
          requestedIndex: propertyIndex,
          propertiesOnPage: response.data.length
        });
      }
      
      const rawProperty = response.data[indexOnPage];
      
      console.log(`🏠 Found property:`, rawProperty._id);
      
      // Return complete raw API data
      res.json({
        success: true,
        requestedIndex: propertyIndex,
        pageInfo: {
        ...
        rawApiData: rawProperty,
        // Also show the converted property for comparison
        convertedProperty: batchLeadsService.convertToProperty(rawProperty, 'debug-user'),
        // Show all available fields in the raw data
        availableFields: {
          topLevel: Object.keys(rawProperty),
          address: rawProperty.address ? Object.keys(rawProperty.address) : [],
          building: rawProperty.building ? Object.keys(rawProperty.building) : [],
          valuation: rawProperty.valuation ? Object.keys(rawProperty.valuation) : [],
          owner: rawProperty.owner ? Object.keys(rawProperty.owner) : [],
          quickLists: rawProperty.quickLists ? Object.keys(rawProperty.quickLists) : [],
          foreclosure: rawProperty.foreclosure ? Object.keys(rawProperty.foreclosure) : [],
          openLien: rawProperty.openLien ? Object.keys(rawProperty.openLien) : []
        }
      });
