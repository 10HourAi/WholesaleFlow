...
Focus on summary, ARV estimate, max offer estimate, deal/no-deal, confidence, notes.`
  ...
  // Run Comps Analysis for a Property
  app.post("/api/properties/:id/comps", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const propertyId = req.params.id;
      
      // Get the property to analyze
      const property = await storage.getProperty(propertyId, userId);
      if (!property) {
        return res.status(404).json({ message: "Property not found" });
      }

      console.log(`🏠 Running comps analysis for property: ${property.address}, ${property.city}, ${property.state}`);

      // For now, create mock comparable sales data
      // TODO: Replace with real comps API (Rentspree, RentBerry, or similar)
      const mockComps = [
        {
          compAddress: `${Math.floor(Math.random() * 900) + 100} ${property.address.split(' ').slice(1).join(' ')}`,
          compCity: property.city,
          compState: property.state,
          compZipCode: property.zipCode,
          saleDate: new Date(Date.now() - Math.random() * 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          salePrice: Math.floor(Math.random() * 100000) + 200000,
          bedrooms: property.bedrooms || Math.floor(Math.random() * 4) + 2,
          bathrooms: property.bathrooms || Math.floor(Math.random() * 3) + 1,
          squareFeet: property.squareFeet || Math.floor(Math.random() * 1000) + 1200,
          yearBuilt: property.yearBuilt || Math.floor(Math.random() * 50) + 1970,
          distanceMiles: +(Math.random() * 2).toFixed(2),
          daysOnMarket: Math.floor(Math.random() * 120) + 15,
          source: "mock_mls",
          confidence: +(Math.random() * 0.3 + 0.7).toFixed(2), // 0.7 - 1.0
          isActive: true,
          notes: "Mock comparable sale for development"
        },
        {
          compAddress: `${Math.floor(Math.random() * 900) + 100} ${property.address.split(' ').slice(1).join(' ')}`,
          compCity: property.city,
          ...
          compZipCode: property.zipCode,
          saleDate: new Date(Date.now() - Math.random() * 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          salePrice: Math.floor(Math.random() * 100000) + 180000,
          bedrooms: property.bedrooms || Math.floor(Math.random() * 4) + 2,
          bathrooms: property.bathrooms || Math.floor(Math.random() * 3) + 1,
          squareFeet: property.squareFeet || Math.floor(Math.random() * 1000) + 1200,
          yearBuilt: property.yearBuilt || Math.floor(Math.random() * 50) + 1970,
          distanceMiles: +(Math.random() * 1.5).toFixed(2),
          daysOnMarket: Math.floor(Math.random() * 90) + 20,
          source: "mock_mls",
        ...
        },
        {
          ...
          compZipCode: property.zipCode,
          saleDate: new Date(Date.now() - Math.random() * 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          salePrice: Math.floor(Math.random() * 120000) + 220000,
          bedrooms: property.bedrooms || Math.floor(Math.random() * 4) + 2,
          bathrooms: property.bathrooms || Math.floor(Math.random() * 3) + 1,
          squareFeet: property.squareFeet || Math.floor(Math.random() * 1000) + 1200,
          yearBuilt: property.yearBuilt || Math.floor(Math.random() * 50) + 1970,
          distanceMiles: +(Math.random() * 2.5).toFixed(2),
          daysOnMarket: Math.floor(Math.random() * 150) + 10,
          source: "mock_mls",
          confidence: +(Math.random() * 0.3 + 0.7).toFixed(2),
          isActive: true,
          notes: "Mock comparable sale for development"
      ...
      const savedComps = [];
      for (const compData of mockComps) {
        const comp = await storage.createComp({
          propertyId,
          ...compData,
          pricePerSqFt: compData.salePrice / compData.squareFeet
        });
        savedComps.push(comp);
      }

      // Calculate comps summary stats
      const avgPrice = mockComps.reduce((sum, comp) => sum + comp.salePrice, 0) / mockComps.length;
      const avgPricePerSqFt = mockComps.reduce((sum, comp) => sum + (comp.salePrice / comp.squareFeet), 0) / mockComps.length;
      const avgDaysOnMarket = mockComps.reduce((sum, comp) => sum + comp.daysOnMarket, 0) / mockComps.length;

      const compsSummary = {
        totalComps: savedComps.length,
        avgSalePrice: Math.round(avgPrice),
        avgPricePerSqFt: Math.round(avgPricePerSqFt),
        avgDaysOnMarket: Math.round(avgDaysOnMarket),
        priceRange: {
          min: Math.min(...mockComps.map(c => c.salePrice)),
          max: Math.max(...mockComps.map(c => c.salePrice))
        },
        analysisDate: new Date().toISOString(),
        confidence: +(mockComps.reduce((sum, comp) => sum + comp.confidence, 0) / mockComps.length).toFixed(2)
      };

      console.log(`✅ Generated ${savedComps.length} comps for property ${propertyId}`);
      ...
      res.json({
        ...
        message: `Found ${savedComps.length} comparable sales within 2.5 miles`
      });
      ...
      res.status(500).json({ 
        success: false,
        message: error.message || "Failed to generate comps analysis"
      });
    }
  });

  // Get existing comps for a property
  app.get("/api/properties/:id/comps", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const comps = await storage.getCompsByProperty(propertyId);
      
      // Calculate summary if comps exist
      let summary = null;
      if (comps.length > 0) {
        const avgPrice = comps.reduce((sum, comp) => sum + (comp.salePrice || 0), 0) / comps.length;
        const avgPricePerSqFt = comps.reduce((sum, comp) => sum + Number(comp.pricePerSqFt || 0), 0) / comps.length;
        const avgDaysOnMarket = comps.reduce((sum, comp) => sum + (comp.daysOnMarket || 0), 0) / comps.length;
        ...
        summary = {
          ...
          avgDaysOnMarket: Math.round(avgDaysOnMarket),
          priceRange: {
            min: Math.min(...comps.map(c => c.salePrice || 0)),
            max: Math.max(...comps.map(c => c.salePrice || 0))
          }
        };
      }
      ...
      res.json({
        ...
        comps,
        summary,
        message: comps.length > 0 ? `Found ${comps.length} existing comps` : "No comps found for this property"
      });
      ...
      res.status(500).json({ 
        success: false,
        message: error.message || "Failed to fetch comps"
      });
    }
  });
